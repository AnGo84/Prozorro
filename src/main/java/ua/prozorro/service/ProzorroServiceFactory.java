...
public class ProzorroServiceFactory {
	...
	public static ParsingResultData getApproximatelyParsingTimeForPeriod(PropertyFields propertyFields, DataType dataType, Date dateFrom, Date dateTill)
		...
		if (dataType.equals(DataType.TENDERS)) {
			...
			return pageServiceProzorro
					.getApproximatelyParsingTimeForPeriod(propertyFields.getSearchDateType(), dateFrom, dateTill);
	...
	public static void parseAndSaveData(SessionFactory sessionFactory, Properties properties, Date dateFrom, Date dateTill) {
		...
		TenderDataServiceProzorro tenderDataServiceProzorro = new TenderDataServiceProzorro(
				properties.getProperty(AppProperty.TENDER_START_PAGE.getPropertyName()) + "/");
