...
public class SchemaExportDialogController {
	...
	public void onButtonOk(ActionEvent actionEvent) {
		...
		if (textFieldFileName.getText() != null && !textFieldFileName.getText().equals("")) {
			...
			if (tempDialectEngine != null && !tempDialectEngine.toUpperCase().equals(HibernateSession.HIBERNATE_DIALECT_STORAGE_ENGINE.toUpperCase())) {
			...
			try {
				...
				Dialogs.showMessage(Alert.AlertType.INFORMATION, new DialogText("Экспорт схемы БД", "Схема БД сохранена в файл '" + file.getName() + "'",
						"Схема для БД '" + comboBoxDB.getValue() + "' сохранена в файл '" + file.getAbsolutePath() + "'"), logger);

				if (checkBoxSaveScheme.isSelected()) {
					if (Dialogs.showConfirmDialog(
							new DialogText("Создать схему БД", "Создать схему БД " + comboBoxDB.getValue(), "Все данные будут удалены! Продолжить?"))) {
						...
						Dialogs.showMessage(Alert.AlertType.INFORMATION, new DialogText("Создание схемы БД", "Схема БД создана",
								"Схема для БД '" + comboBoxDB.getValue() + "' создана '"), logger);
					}
				...
				Dialogs.showErrorDialog(e, new DialogText("Ошибка экспорта", "Ошибка файла конфигураци",
						"Ошибка экспорта схемы БД '" + comboBoxDB.getValue() + "' из файла " + comboBoxDB.getValue().getConfigFileName()), logger);
			} catch (RuntimeException e) {
				Dialogs.showErrorDialog(e, new DialogText("Ошибка экспорта", "Ошибка выполнения экспорта схемы БД",
						"Ошибка экспорта схемы БД '" + comboBoxDB.getValue() + "' в файл " + textFieldFileName.getText()), logger);
			} catch (Exception e) {
			...
			Dialogs.showMessage(Alert.AlertType.WARNING, new DialogText("Ошибка файла", "Ошибка при проверке файла '" + textFieldFileName.getText() + "'",
					"Не верное имя или путь к файлу '" + textFieldFileName.getText() + "'"), logger);
		}
