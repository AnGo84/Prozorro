...
public class ProzorroPageContentToDataPageMapper extends AbstractMapper<ProzorroPageContent, DataPage> {
	...
	public DataPage convertToEntity(ProzorroPageContent object) {
		...
		DataPage dataPage = DataPage.builder().currentDataURL(
				prozorroPageToDataURLMapper.convertToEntity(object.getCurrentPage())).prevDataURL(
				prozorroPageToDataURLMapper.convertToEntity(object.getPrevPage())).nextDataURL(
				prozorroPageToDataURLMapper.convertToEntity(object.getNextPage())).pageContentData(
				contentDataListMapper.convertToEntity(object.getPageElementList())).build();
		return dataPage;
	}
