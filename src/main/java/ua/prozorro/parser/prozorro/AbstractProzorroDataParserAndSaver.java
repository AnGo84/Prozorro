...
public abstract class AbstractProzorroDataParserAndSaver extends AbstractDataParserAndSaver {
	...
	protected boolean prozorroExportJSONOnly;
	...
	public AbstractProzorroDataParserAndSaver(PropertyFields propertyFields) {
		...
		this.prozorroExportJSONOnly =
				(propertyFields.getPropertiesStringValue(AppProperty.PROZORRO_EXPORT_JSON_ONLY) != null &&
				 propertyFields.getPropertiesStringValue(AppProperty.PROZORRO_EXPORT_JSON_ONLY).equals("true"));
	}
	...
	public List<PageElement> getPageElementList() {
		List<PageElement> pageElements =
				prozorroPageElementListMapper.convertToEntitiesList(pageContent.getPageElementList());
		return pageElements;
	}
	
	@Override
	public EventResultData checkExpireElement(PageElement pageElement) throws ParseException {
		Date pageElementDate = DateUtils.parseProzorroPageDateFromString(pageElement.getDateModified(), propertyFields
		...
		if (propertyFields.getSearchDateTill().
			...
			eventResultData.setEventResult(
					...
					pageElement.getId() + ", date: " + pageElement.getDateModified() + ". Отклонён по дате \n");
		} else {
