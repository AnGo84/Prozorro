...
class TenderServiceTest {
	...
	private static TenderService tenderService;
	
	@BeforeAll
	public static void before() {
		sessionFactory = TestHibernateUtils.getTestSessionFactory();
		session = sessionFactory.openSession();
		tenderService = new TenderService(sessionFactory);
	}
	...
	@Test
	public void whenSaveListTender() throws IOException {
		
		tenderService.saveOrUpdate(getTestContentDataList());
		
		List<TenderJSON> Tenders = TestHibernateUtils.getAllTenderJSONs(session);
		assertNotNull(Tenders);
		assertEquals(3, Tenders.size());
	}
	
	@Test
	public void whenSaveEmptyListTender() throws IOException {
		List<ActionResult> actionResults = tenderService.saveOrUpdate(null);
		...
		actionResults = tenderService.saveOrUpdate(new ArrayList<>());
		assertNotNull(actionResults);
		assertFalse(actionResults.isEmpty());
		assertTrue(actionResults.size() == 1);
		assertEquals(ResultType.ERROR, actionResults.get(0).getResultType());
	...
	@Test
	public void whenSaveListTenderThrowsException() {
		assertThrows(NullPointerException.class, () -> {
			tenderService.saveOrUpdate(Arrays.asList(new ContentData()));
		});
		...
		assertThrows(PersistenceException.class, () -> {
			tenderService.saveOrUpdate(Arrays.asList(contentData));
		});
	}
